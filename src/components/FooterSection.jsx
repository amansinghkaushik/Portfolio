function FooterSection() {
  return (
    <footer className="bg-black px-6 pb-12 pt-16 text-white sm:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        <div className="mb-20 flex justify-end gap-8 text-sm text-white/75">
          <a href="#" className="hover:text-white">
            Linkedin
          </a>
          <a href="#" className="hover:text-white">
            Twitter
          </a>
          <a href="#" className="hover:text-white">
            Behance
          </a>
        </div>

        <h3 className="mb-10 max-w-4xl text-4xl font-semibold leading-tight text-white/95 sm:text-5xl">
          Curious about what we can create together? Let's bring something extraordinary to life.
        </h3>

        <div className="mb-20 flex flex-wrap items-center gap-8">
          <button
            type="button"
            className="rounded-none bg-white px-7 py-3 text-sm font-semibold text-black hover:bg-[#e7e7e7]"
          >
            Get in Touch
          </button>
          <p className="text-sm text-white/80">● Available For Work</p>
        </div>

        <div className="grid gap-6 text-sm text-white/70 sm:grid-cols-3">
          <div>
            <p>+81 (0)90 1234 5678</p>
            <p>hello@yuya.com</p>
          </div>
          <p>Designed & Developed by Peter Hodak</p>
          <p className="sm:text-right">All rights reserved, YUYA © 2024</p>
        </div>
      </div>
    </footer>
  )
}

export default FooterSection
